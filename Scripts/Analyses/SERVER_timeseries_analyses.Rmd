...
Trial running a set of points with parallelizing on server
```{r}
#Data input
...
```
Above run was a success. In next chunk running classification analyses using og_anisoEVI values for all points
```{r}
rds_list <- list.files(path = here("Outputs", "STL_Decomposition",
                                   "Split_datagroups", "Split_datagroups"), 
                       pattern='.rds$', all.files=TRUE, full.names=TRUE)
rds_list<-gtools::mixedsort(rds_list) #arranging chronologically
...
write_rds(x, here("Outputs", "Trajectory_Classification", "OGanisoEVI_Classification_df", "classification_1701_1804.rds"))
...
Sys.time(); for (i in 101:200){ #2 hours
...
In the next chunk I do model selection of the models in which I have used 
original anisoEVI
```{r}
rds_list <- list.files(path = here("Outputs", "Trajectory_Classification","OGanisoEVI_Classification_df"), 
                       pattern='.rds$', all.files=TRUE, full.names=TRUE)
rds_list <- gtools::mixedsort(rds_list) #arranging chronologically

#rerun lines 1012- 1024 to set up the parallelization
...
write_rds(y, here("Outputs", "Trajectory_Classification", "OGanisoEVI_Final_model_df", "modelselect.rds"))
...
rds_list2 <- list.files(path = here("Outputs", "Trajectory_Classification","Final_model_df"), 
                       pattern='.rds$', all.files=TRUE, full.names=TRUE)
...
ggsave(here("Outputs", "Trajectory_Classification", "trajectory_prop_plot.png"), plot = plot_prop_traj,
       height = 10, width = 10, units = "cm", dpi=700)  


### Map making
Sys.time(); map_raster<- selectmodel_df %>% ungroup() %>%
  dplyr::select(c(x, y, model_order, shape_class)) %>% 
  mutate(pixelvalue= case_when(model_order=="Null" & shape_class== "stable_constant"~1,
                               model_order=="Lin" & shape_class== "decrease_constant"~2,
                               model_order=="Lin" & shape_class== "inrease_constant"~3,
                               model_order=="Lin" & shape_class== "stable_constant"~4,
...
modelselect_map<-#tm_shape(neighbors, bbox = map_extent)+ tm_borders()+
  #tm_shape(d_trans)+ tm_fill()+
  tm_shape (map_raster)+
  tm_raster(col = "pixelvalue", style = "cat", palette =MyPalette, legend.show = F) +
  tm_layout(legend.text.size = 1.5)+
  tm_add_legend(type = "fill", 
                labels = c("Null-Stable,Constant", 
                           "Linear- Decrease, Constant",
                           "Linear- Increase, Constant",
                           "Linear- Stable, Constant",
                           "Quadratic- Stable, Concave",
                           "Quadratic- Stable, Convex"),
                col = MyPalette)
tmap_save(modelselect_map, here("Outputs", "Trajectory_Classification", "trajectory_map.png"),
          height = 30, width = 30, units = "cm", dpi=700)

remove(map_raster, plot_prop_traj, prop_traj, neighbors, map_extent, d_trans, modelselect_map)
remove(MyPalette, rds_list2)
remove(df_list, selectmodel_df,i)
```


##############################################5. Results interpretation
Here I look under the hood of the results. basically the resutls are that there
...
lineplot_func<- function(cellvalue){
