...
tic(); aniso_evi<- aniso_evi/10^5; toc() #scaling factor mentioned on the description page of the GEE asset
...
evi_stack<- function (Year_Month){
  terra::subset(aniso_evi, grep(Year_Month, names (aniso_evi)))
}
...
tic(); mask_df<-mask_df %>% mutate(masked_EVI = purrr::map2(monthlyanisoEVI, mask, masking_function)); toc() #71 min
...
Then I find the amount of area remaining i.e total area of pixels within which trends will be anlaysed. I present this a s a proportion of the total cerrado area
```{r}
...
tic(); area_mask_df<-area_mask_df %>% mutate(masked_EVI_area_sqkm = map(monthlyarea, sum_area)); toc() #21 min
...
tmap_save(coverage_map, here("Outputs", "Indices","Masked_anisoEVI", "map_coverage_after_maskingLULC.png"))
...
I dig into the NAs more in the next chunk. Based on conversation with Josh on 21st Feb- I will find (1) in each year, ho wmany months of consecutive NAs are present for a pixel. 
...
for(i in 1:length(years)){
  x<- subset(masked_evi, grep(years[i], names(masked_evi)))
...
x<-tmap_arrange(number_consecutive_map_list[[21]])
tmap_save(x, here("Outputs","Indices", "Masked_anisoEVI","ConsecutiveNA_inyear_throughmonths", "year_consecutiveNA21.png"), width = 1000, height = 750, dpi = 300)
