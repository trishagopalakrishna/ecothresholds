...
```{r}
m <- rbind(c(0, 0), c(1, 1), c(2, 2), c(3, 3), c(4, 4))
...
tic(); df_no2001_0_4maxconsecutiveNA<- as.data.frame(all_rasters, xy=T, cell=T); toc()  
#write_rds(df_no2001_max_consecutiveNAs, here("Outputs", "Pixels_of_Interest", "df_no2001_0_4maxconsecutiveNA.rds"))
names(df_no2001_max_consecutiveNAs)

tic(); pivot_df_trial<- pivot_longer(df_no2001_0_4maxconsecutiveNA, 4:243); toc()
#write_rds(df_no2001_max_consecutiveNAs, here("Outputs", "Pixels_of_Interest", "pivot_df_no2001_0_4maxconsecutiveNA.rds"))


tic(); linear_int_no2001_0_4maxconsecutiveNA<- pivot_df_trial %>%
  group_by(cell) %>% filter(max==1); toc()  #NAs :2115246 in value column


library(imputeTS)
tic(); linear_int_no2001_0_4maxconsecutiveNA <- linear_int_no2001_0_4maxconsecutiveNA %>%
  group_by(cell) %>%
  mutate(value_int = imputeTS::na_interpolation(value, option = "linear", maxgap = 1));toc()
#tic(); write_rds(linear_int_no2001_0_4maxconsecutiveNA, here("Outputs", "Pixels_of_Interest", "linear_int_no2001_0_4maxconsecutiveNA.rds"));toc()

nomax1_pivot_df_trial<- pivot_df_trial %>% group_by(cell) %>% filter(max!=1)
tic(); write_rds(nomax1_pivot_df_trial, here("Outputs", "Pixels_of_Interest", "nomax1_pivot_df_trial.rds"));toc()
