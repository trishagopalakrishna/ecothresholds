...
First, creating random points in pixels wiith a full time series
```{r}
...
scenario1<- df_timeseries_NA_sample2 %>% group_by(Year) %>% mutate(value= ifelse(Month=="4" | Month=="5", NA, value)) 
...
for (i in 1:(dim(NA_position)[1])){
  scenario2<- scenario2 %>% mutate(value= ifelse(Year==NA_position[i,1] & (Month==NA_position[i,2] | Month== NA_position[i,3]),NA, value))
...
##Decision 2- Exclude all pixels that have ALL anisoEVI values through time are missing/are NA
```{r}
