...
anisoEVI_random_points<- anisoEVI_random_points %>% mutate(na_count= rowSums(is.na(select(.,-c(id, PointID, ID_1, region, x, y))))) %>% filter(na_count!=251) %>% dplyr::select(-na_count) 
...
for (i in 1:dim(anisoEVI_random_points)[1]){
  ...
  one_point<- anisoEVI_random_points[i,] %>% dplyr::select(-c(id, PointID, ID_1, region, x, y))
...
x <- newformat %>% dplyr::filter(PointID==1) #trial
