...
class_trajectory_mod <- function (Y = NULL, X = NULL, dataset = NULL,
  ...
  # # Significance of coeeficients  
  # if(summary(orth_poly_mod)$coefficients[3, 4] <= 0.05){
  #   classification$best_model <- c("best", NA, NA) # quadratic model
  # } else if(summary(orth_poly_mod)$coefficients[2, 4]<=0.05){
  ...
  linear.model.summary <- summary(linear_mod)
...
#Trial above classification function (null, linear, quadratic model)
```{r}
#Data input
...
chng_fit<- chngpt::chngptm(formula.1=trend~1, 
                      formula.2=~Months, 
                      family="gaussian", data=trial_df,
                      type="step", 
                      var.type="bootstrap", weights=NULL)

pred_chg <- data.frame(year = trial_df$Months,
                         bp = chng_fit$best.fit$fitted.values)

nrmse <- sqrt(sum(residuals(chng_fit)^2)/length(trial_df$trend))/sd(trial_df$trend)

# Summarize chngt outputs in one line:
chg_out <- data.frame(abbr = "chg",
                        mtd = "chgnpt",
                        n_brk = 1,
                        loc_brk = chng_fit$chngpt,
                        aic = MuMIn::AICc(chng_fit),
