...
tic(); for (i in 259:length(rds_list)){
...
rds_list2 <- list.files(path = here("Outputs", "Trajectory_Classification","Final_model_combined_ogdata_df"), 
                       pattern='.rds$', all.files=TRUE, full.names=TRUE)
rds_list2<-gtools::mixedsort(rds_list2)
df_list<- list()
for ( i in 1:length(rds_list2)){
...
selectmodel_df%>% group_by(model_order) %>% 
  summarise(count= n()/240) %>% 
  mutate(percentage= (count/5000)*100)

map_df<- selectmodel_df %>% 
  dplyr::select(c(x.x, y.x, model_order)) %>% 
  distinct(x.x, y.x, .keep_all = TRUE) %>%
  st_as_sf(coords = c("x.x", "y.x"))
map_df<- map_df %>% mutate(pixelvalue= case_when(model_order=="Null"~1,
                                                 model_order=="Lin"~2,
                                                 model_order=="Quad"~3))
...
modelselect_map<-tm_shape(neighbors, bbox = map_extent)+ tm_borders()+
  tm_shape(d_trans)+ tm_fill()+
  tm_shape (map_df)+
  tm_dots(col = "pixelvalue", palette = c(Null= "red", Lin= "blue", Quad="black"))

remove(map_df, neighbors, map_extent, d_trans, modelselect_map)
