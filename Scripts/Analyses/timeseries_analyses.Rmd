...
tic(); for (i in 795:length(rds_list)){
...
### Bar plot
prop_traj<-selectmodel_df%>% group_by(model_order, shape_class) %>% 
  summarise(count= n()) %>%
  mutate(percentage= (count/nrow(selectmodel_df))*100) %>%
  unite(Model_Shape, c("model_order", "shape_class"))

MyPalette <- c(Null_stable_constant ="#CCCCCC", 
               Lin_decrease_constant= "#332288", 
               Lin_increase_constant= "#117733", 
               Lin_stable_constant= "#CC6677",
               Quad_stable_concave="#AA4499",
               Quad_stable_convex="#882255")

plot_prop_traj<- 
  ggplot(prop_traj, aes(x=Model_Shape, y= percentage, fill=Model_Shape)) +
...
Sys.time(); for (i in 1:10){ #2 hours
  test_df<- read_rds(rds_list[[i]])
  names(test_df)[15]<- "trend_name"
  plan(multisession, workers = numCores)
  tic()
...
plot_prop_traj<- 
  ...
  theme_classic()+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1),legend.position="none")
ggsave(here("Outputs", "Trajectory_Classification", "Final_model_combined_ogdata_df", "oganisoEVI_trajectory_prop_plot.png"), plot = plot_prop_traj,
       height = 30, width = 30, units = "cm", dpi=700)  


# #Quad pixels
# quad_pixels<- selectmodel_df %>% filter(model_order=="Quad") 
# 
# eg_quad_pixel<- quad_pixels %>% 
#   filter(cell==113954)
# plot(eg_quad_pixel$og_anisoEVI, type="l")
# 
# #Lin pixels
# lin_pixels<- selectmodel_df %>% filter(model_order=="Lin") 
# 
# eg_lin_pixel<- lin_pixels %>% 
#   filter(cell==124023)
# plot(eg_lin_pixel$trend, type="l")
# 
# #Null pixels
# null_pixels<- selectmodel_df %>% filter(model_order=="Null") 
# 
# eg_null_pixel<- null_pixels %>% 
#   filter(cell==126268)
# plot(eg_null_pixel$trend, type="l")
