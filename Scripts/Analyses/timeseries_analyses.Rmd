...
tic(); for (i in 1:length(rds_list)){
  test_df<- read_rds(rds_list[[1]])
  plan(multisession, workers = numCores)
  tic()
  x <- test_df %>%
    group_by(cell) %>%
    nest() %>%
    mutate(classification_data = furrr::future_map(data, class_trajectory_mod, .options = furrr_options(seed = TRUE))) %>%
    select(-data) %>%
    unnest(classification_data)
  toc()
  write_rds(x, here("Outputs", "Trajectory_Classification", "Classification_df", paste0("classification",i,".rds")))
  remove(test_df, x)
