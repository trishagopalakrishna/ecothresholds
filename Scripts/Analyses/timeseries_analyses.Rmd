...
>>>>>>> 6fe5b3e2295b1fef10200a022a45f676c51b92b6
...
# ## Plain nested for loop
# for ( i in 1:length(rds_list)){
#   print (i)
#   test_df<- read.rds(rds_list[[i]])
#   x1000cell<- list()
#   cell_id<- unique(test_df$cell)
#     for (j in 1:length(cell_id)){
#       one_cell<- test_df %>% filter(cell==cell_if[j])
#       one_cell_results<- class_trajectory_mod(one_cell)
#       x1000cell[j]<- one_cell_results
#       remove(one_cell_results, one_cell)
#       gc()
#     }
#   x1000cell_df<- do.call(rbind, x1000cell)
#   write_rds(x1000cell_df, here("Outputs", "Trajectory_Classification", "Classification_df", 
#                     paste0("classification",name_i[i],".rds")))
#   remove(x1000cell,x1000cell_df, test_df, cell_id)
#   print ("Next rds file")
#   gc()
# }
...
#detrended_rds_list<- detrended_rds_list[-1]
#df_list<- list()
...
selectmodel_df<-readRDS(here("Outputs","Archive", "Trajectory_Classification","Final_model_df", "combined_modelselect.rds"))

oganisoEVI <- readRDS(here("Outputs","Archive", "Trajectory_Classification", "OGanisoEVI_Final_model_df", "modelselect.rds"))
...
modelselect_map<-#tm_shape(neighbors, bbox = map_extent)+ tm_borders()+
  ...
  tm_add_legend(type = "fill", 
                ...
                col = Set1palette)
tmap_save(modelselect_map, here("Outputs", "Trajectory_Classification", "detrended_trajectory_map_grey.png"),
          height = 30, width = 30, units = "cm", dpi=700)

ogSet1palette<- brewer.pal(n=8, "Set1")
ogSet1palette[[4]]<- "grey" 
ogSet1palette[[3]]<- "grey" 
...
Sys.time(); og_map_raster<- oganisoEVI %>% ungroup() %>%
  ...
  mutate(pixelvalue= case_when(model_order=="Lin" & shape_class== "decrease_constant"& is.na(trend_name)~1,
                              ...
                              model_order=="Step" & is.na(shape_class) & trend_name == "decrease" ~7,
                              model_order=="Step" & is.na(shape_class) & trend_name == "increase" ~8)) %>%
...
og_modelselect_map<-#tm_shape(neighbors, bbox = map_extent)+ tm_borders()+
  ...
  tm_add_legend(type = "fill", 
                labels = c("Linear- Decrease, Constant", 
                           ...
                           "Step- Decrease"),
                col = ogSet1palette)
tmap_save(og_modelselect_map, here("Outputs", "Trajectory_Classification", "og_trajectory_map_grey.png"),
          height = 30, width = 30, units = "cm", dpi=700)
...
#4.Percentage area/number of pixels that have "unstable" trajectories 
#in each of the climate zones by Marina's collegaues 
d_trans<- st_read(here ("Data", "Cattelanetal_Clustering", "cerrado_climate_zones.shp"))
d_trans<- st_transform(d_trans, crs = 4326)
crs(og_map_raster)<- crs(d_trans)
crs(detrended_map_raster)<- crs(d_trans)

central<- d_trans %>% filter(region=="Central")
southern<- d_trans %>% filter(region=="Southern")
eastern<- d_trans %>% filter(region=="Eastern")

area_zone_tabulation_function<- function(shapefile, pixeltype_raster, zone_name){
  subset_ra <- mask(pixeltype_raster, shapefile)
  df <- as.data.frame(table(as.matrix(subset_ra)))
  df$Percent <- df$Freq / sum(df$Freq) * 100
  df$ZoneName <- zone_name
  df
}

central_og<- area_zone_tabulation_function(central, og_map_raster, "Central")
central_og<- central_og %>% mutate(DataType="OG_anisoEVI")
southern_og<- area_zone_tabulation_function(southern, og_map_raster, "Southern")
southern_og<- southern_og %>% mutate(DataType="OG_anisoEVI")
eastern_og<- area_zone_tabulation_function(eastern, og_map_raster, "Eastern")
eastern_og<- eastern_og%>% mutate(DataType="OG_anisoEVI")

central_detrended<- area_zone_tabulation_function(central, detrended_map_raster, "Central")
central_detrended<- central_detrended %>% mutate(DataType="Decomposed_anisoEVI")
southern_detrended<- area_zone_tabulation_function(southern, detrended_map_raster, "Southern")
southern_detrended<- southern_detrended %>% mutate(DataType="Decomposed_anisoEVI")
eastern_detrended<- area_zone_tabulation_function(eastern, detrended_map_raster, "Eastern")
eastern_detrended<- eastern_detrended %>% mutate(DataType="Decomposed_anisoEVI")

central_trajectories<- rbind(central_og, central_detrended)
southern_trajectories<- rbind(southern_og, southern_detrended)
eastern_trajectories<- rbind(eastern_og, eastern_detrended)

central_trajectories<- central_trajectories %>% mutate(Traj_type= case_when(Var1==1~"Lin_decrease_constant",
                                                                        Var1==2~"Lin_increase_constant",
                                                                        Var1==3~"Lin_stable_constant",
                                                                        Var1==4~"Null_stable_constant",
                                                                        Var1==5~"Quad_stable_concave",
                                                                        Var1==6~"Quad_stable_convex",
                                                                        Var1==7~"Step_decrease",
                                                                        Var1==8~"Step_increase"))
  

southern_trajectories<- southern_trajectories %>% mutate(Traj_type= case_when(Var1==1~"Lin_decrease_constant",
                                                                        Var1==2~"Lin_increase_constant",
                                                                        Var1==3~"Lin_stable_constant",
                                                                        Var1==4~"Null_stable_constant",
                                                                        Var1==5~"Quad_stable_concave",
...
eastern_trajectories<- eastern_trajectories %>% mutate(Traj_type= case_when(Var1==1~"Lin_decrease_constant",
                                                                        Var1==2~"Lin_increase_constant",
                                                                        Var1==3~"Lin_stable_constant",
                                                                        Var1==4~"Null_stable_constant",
                                                                        Var1==5~"Quad_stable_concave",
...
#Barplot
x1<-central_trajectories %>% 
  ggplot(aes(DataType, Percent, fill = Traj_type, alpha = DataType)) +
  geom_col(position = position_dodge(), color = "black") +
  theme_classic() +
  scale_alpha_manual(values = c(0.5, 1), guide = guide_none()) +
  facet_grid(~Traj_type, scales = "free_x", switch = "x") +
  theme(strip.placement  = "outside",
        panel.spacing    = unit(0, "points"),
        strip.background = element_blank(),
        strip.text       = element_text(size = 12, angle = 90),
        axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1),
        legend.position = "none")
x1<- x1 + scale_fill_brewer(palette = "Set1")
ggsave(here("Outputs", "Trajectory_Classification", "central_trajectories.png"), plot=x1,height = 30, width = 30, units = "cm", dpi=700 )

x2<-southern_trajectories %>% 
  ggplot(aes(DataType, Percent, fill = Traj_type, alpha = DataType)) +
  geom_col(position = position_dodge(), color = "black") +
  theme_classic() +
  scale_alpha_manual(values = c(0.5, 1), guide = guide_none()) +
  ...
  theme(strip.placement  = "outside",
        ...
        axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1),
        legend.position = "none")
x2<- x2 + scale_fill_brewer(palette = "Set1")
ggsave(here("Outputs", "Trajectory_Classification", "southern_trajectories.png"), plot=x2,height = 30, width = 30, units = "cm", dpi=700 )

x3<-eastern_trajectories %>% 
  ggplot(aes(DataType, Percent, fill = Traj_type, alpha = DataType)) +
  geom_col(position = position_dodge(), color = "black") +
  theme_classic() +
  scale_alpha_manual(values = c(0.5, 1), guide = guide_none()) +
  ...
  theme(strip.placement  = "outside",
        ...
        axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1),
        legend.position = "none")
x3<- x3 + scale_fill_brewer(palette = "Set1")
ggsave(here("Outputs", "Trajectory_Classification", "eastern_trajectories.png"), plot=x3,height = 30, width = 30, units = "cm", dpi=700 )
