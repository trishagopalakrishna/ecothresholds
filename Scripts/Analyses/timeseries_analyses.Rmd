...
trial_tsibble %>%
 model(STL(value_int ~ trend(window = 7) +
  ...
  components() %>%
  autoplot();toc()
###################### ABOVE NOT WORKING ####################### Cannot figure out why

###################### ALTERNATIVE BELOW WORKING #######################
trial_ts<- ts(trial$value_int, start=c(2001,1), end=c(2021,12), frequency=12)
 plot(stl(trial_ts, s.window = "periodic", t.window = 7, t.jump = 1))
tic(); trial_tsibble %>% 
...
##############################################7. Regression type analyses ie not doing any sort of decomposition/differencing for removal of seasonality. All data is included (irrespective of seasons) and let regression type of analyses get the trend
