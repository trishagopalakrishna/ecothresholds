...
rds_list <- list.files(path = here("Outputs", "STL_Decomposition","Split_datagroups"), pattern='.rds$', all.files=TRUE, full.names=TRUE)
rds_list<-gtools::mixedsort(rds_list) #arranging chronologically
tic(); above_rds_list<-lapply(rds_list, read_rds); toc() #2 min 

for (i in 1:length(above_rds_list)){
  unique_cell<- above_rds_list[[i]] %>%
    group_by(cell) %>% summarise(mean_value_og= mean(og_anisoEVI)); toc() #23 seconds!
  unique_cell<- unique_cell %>% dplyr::select(cell)
  print ("Unique cell ids from current .rds extracted")
  classification_list<- list()
  print ("Trajectory classification started")
  tic(); for ( j in 1:dim(unique_cell)[1]){
    trial_df<- above_rds_list[[i]] %>% filter(cell==unique_cell[[1]][j])
    trial_df<- trial_df %>% dplyr::select(-c(og_anisoEVI, int_anisoEVI, seasonal, remainder))
    trial_df<- trial_df %>% mutate(Months= 1:nrow(trial_df))
    x<- class_trajectory_mod (trial_df)
    print ("Trajectory classification completed")
    classification_list[[i]]<- x
    print (paste0("Cell id=", j, "/1000"))
  }; toc()
  write_rds(classification, here("Outputs", "Trajectory_Classification", "Classification_df", paste0("classification", i, ".rds")))
  print ("Classification df list written out")
}
