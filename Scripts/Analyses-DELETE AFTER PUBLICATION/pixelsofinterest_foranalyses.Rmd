...
#map of LULC masked EVI 
neighbors<- st_read(here("Data", "Admin", "brazil_neighbors.shp"))

map_extent<- st_bbox(c(xmin=-77.59, xmax=-31.09,
                       ymin=6.42, ymax=-33.49), crs=4326) %>% st_as_sfc()
tic(); maskedevi2_agg<- terra::aggregate(masked_evi2, fact = 3 , fun= "median");toc()


tmap_mode("plot")
maskedevi2_map<-tm_shape(neighbors, bbox = map_extent)+ tm_borders()+
  tm_shape(maskedevi2_agg)+
...
tic();tmap_animation(maskedevi2_map, filename = here("Outputs", "Indices","Masked_anisoEVI", "LULCmasked_anisoEVI_map.gif"),
...
writeRaster(rcx1, here("Outputs",  "0_4_maxconsecutiveNA.tif"))
