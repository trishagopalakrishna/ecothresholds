...
#for SI figure
si_grid_creation_prediction <- function (df_analyses, 
                                      xaxis_variable_in_quotes, 
                                      line_variable_in_quotes, 
                                      gam_model){
  #Creating new data and running model
  ...
  names(low_plot_df)<- c(xaxis_variable_in_quotes, "main_effect", "line_variable_type")
  low_plot_df <- left_join(low_plot_df, ci_xEff, by = xaxis_variable_in_quotes)
  low_plot_df <- low_plot_df %>% mutate(formation_level = paste0("low (",(round(line_variable_type, digits =3)*100), "%)"))
  
  average_plot_df <- plot_df %>% mutate(x1Eff_colname = x2Eff_inv, line_variable_type = line_range[[2]])
  names(average_plot_df)<- c(xaxis_variable_in_quotes, "main_effect", "line_variable_type")
  average_plot_df <- left_join(average_plot_df, ci_xEff_2, by = xaxis_variable_in_quotes)
  average_plot_df <- average_plot_df %>% mutate(formation_level = paste0("average (",(round(line_variable_type, digits =3)*100), "%)"))
  
  high_plot_df <- plot_df %>% mutate(x1Eff_colname = x3Eff_inv, line_variable_type = line_range[[3]])
  names(high_plot_df)<- c(xaxis_variable_in_quotes, "main_effect", "line_variable_type")
  high_plot_df <- left_join(high_plot_df, ci_xEff_3, by = xaxis_variable_in_quotes)
  high_plot_df <- high_plot_df %>% mutate(formation_level = paste0("high (",(round(line_variable_type, digits =3)*100), "%)"))
  
  final_plot_df <- bind_rows(low_plot_df, average_plot_df, high_plot_df)
  final_plot_df 
}
si_greening_anthropicdist_savanna <- si_grid_creation_prediction(analyses_df_noNA_nooutliers, 
                                                           "anthropic_dist", 
                                                           "mean_savanna_percentage",
                                                           inc_gam)
si_greening_anthropicdist_savanna <- si_greening_anthropicdist_savanna %>% mutate(Vegetation ="Savanna")
si_greening_anthropicdist_forest <- si_grid_creation_prediction(analyses_df_noNA_nooutliers, 
                                                           "anthropic_dist", 
                                                           "mean_forest_percentage",
                                                           inc_gam)
si_greening_anthropicdist_forest <- si_greening_anthropicdist_forest %>% mutate(Vegetation ="Forest")
si_greening_anthropicdist_grass <- si_grid_creation_prediction(analyses_df_noNA_nooutliers, 
                                                           "anthropic_dist", 
                                                           "mean_grass_percentage",
                                                           inc_gam)
si_greening_anthropicdist_grass <- si_greening_anthropicdist_grass %>% mutate(Vegetation ="Grass")
si_greening_anthropicdist_veg <- bind_rows(si_greening_anthropicdist_forest, si_greening_anthropicdist_grass, si_greening_anthropicdist_savanna)

si_plot_trend_interaction <- function (interaction_df, xaxis_variable, xlabel){
  x_plot<- ggplot(interaction_df, aes(x = .data[[xaxis_variable]], y = main_effect), color = "#61A36A", fill = "#669900" ) +
    geom_line(color = "#61A36A", linewidth = 1) + 
    geom_ribbon(aes(ymin = lower.ci, ymax = upper.ci), fill = "#669900",color = "#61A36A", alpha = 0.1) +
    coord_cartesian(xlim = c(quantile(interaction_df[[xaxis_variable]], probs=0.1),quantile(interaction_df[[xaxis_variable]], probs=0.9)), ylim = c(0,1)) +
    geom_hline(yintercept= greening_intercept_transposed, linetype='longdash', col = 'grey')+
    facet_wrap(factor(Vegetation, unique(interaction_df$Vegetation))
      ~factor(formation_level, unique(interaction_df$formation_level)), nrow=3 , ncol = 3)+
    theme_classic() +
    ylab ("Probability of greening") + xlab(xlabel)+
    theme(legend.title=element_blank())
  x_plot
}
si_anthropicdist_vegformation <- si_plot_trend_interaction(si_greening_anthropicdist_veg, "anthropic_dist", "distance to anthropic land use (m)")
ggsave (here("Outputs", "AnalysesResults", "GAMResults", "TwoWay_InteractionResults", "si_anthropicdist_vegformation_probability.png"), 
        si_anthropicdist_vegformation,
        dpi = 700, height = 25, width=25, units = "cm")
...
#for SI
si_greening_burnedarea_grass <- si_grid_creation_prediction(analyses_df_noNA_nooutliers, 
                                                           "trend_burnedarea", 
                                                           "mean_grass_percentage",
                                                           inc_gam)
si_plot_greening_burnedarea_grass <- ggplot(si_greening_burnedarea_grass, aes(x = .data[["trend_burnedarea"]], y = main_effect), color = "#61A36A", fill = "#669900" ) +
    geom_line(color = "#61A36A", linewidth = 1) + 
    geom_ribbon(aes(ymin = lower.ci, ymax = upper.ci), fill = "#669900",color = "#61A36A", alpha = 0.1) +
    coord_cartesian(xlim = c(quantile(si_greening_burnedarea_grass[["trend_burnedarea"]], probs=0.1),quantile(si_greening_burnedarea_grass[["trend_burnedarea"]], probs=0.9)), ylim = c(0,1)) +
    geom_hline(yintercept= greening_intercept_transposed, linetype='longdash', col = 'grey')+
    facet_wrap(~factor(formation_level, unique(si_greening_burnedarea_grass$formation_level)), nrow=1)+
    theme_classic() +
    ylab ("Probability of greening") + xlab("trend in burned area (% per year)")+
    theme(legend.title=element_blank())
ggsave (here("Outputs", "AnalysesResults", "GAMResults", "TwoWay_InteractionResults", "si_burnedarea_grass_probability.png"), 
        si_plot_greening_burnedarea_grass,
        dpi = 700, height = 25, width=25, units = "cm")
...
si_grid_creation_prediction <- function (df_analyses, 
  ...
  names(average_plot_df)<- c(xaxis_variable_in_quotes, "main_effect", "line_variable_type")
  average_plot_df <- left_join(average_plot_df, ci_xEff_2, by = xaxis_variable_in_quotes)
  average_plot_df <- average_plot_df %>% mutate(formation_level= paste0("average (", (round(line_variable_type, digits =4)*100), "%)"))
  
  high_plot_df <- plot_df %>% mutate(x1Eff_colname = x3Eff_inv, line_variable_type = line_range[[3]])
  names(high_plot_df)<- c(xaxis_variable_in_quotes, "main_effect", "line_variable_type")
  high_plot_df <- left_join(high_plot_df, ci_xEff_3, by = xaxis_variable_in_quotes)
  high_plot_df  <- high_plot_df  %>% mutate(formation_level = paste0("high (", (round(line_variable_type, digits =4)*100), "%)"))
  
  final_plot_df <- bind_rows(low_plot_df, average_plot_df, high_plot_df)
  final_plot_df <- final_plot_df %>% mutate(line_variable_type2 = paste0(panel_variable_type_dec_nochange_inc_quotes, "(",round(panel_variable_value, digits = 3), ")"))
}

greening_decspei_heatwave_savanna <- si_grid_creation_prediction(analyses_df_noNA_nooutliers, 
                                 "trend_heatwave",
                                 "mean_savanna_percentage",
                                 "trend_spei",
                                 "dec",
...
greening_nochangespei_heatwave_savanna <- si_grid_creation_prediction(analyses_df_noNA_nooutliers, 
                                 "trend_heatwave",
                                 "mean_savanna_percentage",
                                 "trend_spei",
                                 "nochange",
...
greening_spei_heatwave_savanna <- bind_rows(greening_decspei_heatwave_savanna, greening_nochangespei_heatwave_savanna)

remove(greening_decspei_heatwave_savanna, greening_nochangespei_heatwave_savanna)

greening_intercept_transposed <- inv.logit(inc_gam$coefficients[[1]])

si_plot_trend_interaction <- function (interaction_df, xaxis_variable, xlabel){
  x_plot<- ggplot(interaction_df, aes(x = .data[[xaxis_variable]], y = main_effect,
                                          color = line_variable_type2, fill = line_variable_type2,
                                          group = interaction(line_variable_type, line_variable_type2))) +
    geom_line(aes(color = line_variable_type2), linewidth = 1) + 
    geom_ribbon(aes(ymin = lower.ci, ymax = upper.ci), alpha = 0.1) +
    coord_cartesian(xlim = c(quantile(interaction_df[[xaxis_variable]], probs=0.1),quantile(interaction_df[[xaxis_variable]], probs=0.9)), ylim = c(0,1)) +
    geom_hline(yintercept= greening_intercept_transposed, linetype='longdash', col = 'grey')+
    facet_wrap(~factor(formation_level, unique(interaction_df$formation_level)), nrow=1)+
    theme_classic() +
    scale_fill_manual(values = c("#999999", "#660066"))+
    scale_color_manual(values=c("#000000", "#FF99FF")) + 
    ylab ("Probability of greening") + xlab(xlabel)+
    theme(legend.title=element_blank())
  x_plot
}

plot_trend_savanna_heatwave_spei <- si_plot_trend_interaction (greening_spei_heatwave_savanna, "trend_heatwave", "trend in number of heatwaves (per year)")
ggsave (here("Outputs", "AnalysesResults", "GAMResults", "ThreeWay_InteractionResults", "SI_heatwave_spei_savanna.png"), 
        plot_trend_savanna_heatwave_spei,
...
#fig for SI
si_grid_creation_prediction2 <- function (df_analyses, 
                                      xaxis_variable_in_quotes, 
                                      line_variable_in_quotes,
                                      panel_variable_in_quotes,
                                      panel_variable_type_dec_nochange_inc_quotes,
  ...
  names(low_plot_df)<- c(xaxis_variable_in_quotes, "main_effect", "line_variable_type")
  low_plot_df <- left_join(low_plot_df, ci_xEff, by = xaxis_variable_in_quotes)
  low_plot_df <- low_plot_df %>% mutate(formation_level = paste0("low (",(round(line_variable_type, digits =2)*100), "%)"))
  
  average_plot_df <- plot_df %>% mutate(x1Eff_colname = x2Eff_inv, line_variable_type = line_range[[2]])
  names(average_plot_df)<- c(xaxis_variable_in_quotes, "main_effect", "line_variable_type")
  average_plot_df <- left_join(average_plot_df, ci_xEff_2, by = xaxis_variable_in_quotes)
  average_plot_df <- average_plot_df %>% mutate(formation_level= paste0("average (",(round(line_variable_type, digits =2)*100), "%)"))
  
  high_plot_df <- plot_df %>% mutate(x1Eff_colname = x3Eff_inv, line_variable_type = line_range[[3]])
  names(high_plot_df)<- c(xaxis_variable_in_quotes, "main_effect", "line_variable_type")
  high_plot_df <- left_join(high_plot_df, ci_xEff_3, by = xaxis_variable_in_quotes)
  high_plot_df  <- high_plot_df  %>% mutate(formation_level = paste0("high (",(round(line_variable_type, digits =2)*100), "%)"))
  
  final_plot_df <- bind_rows(low_plot_df, average_plot_df, high_plot_df)
  final_plot_df <- final_plot_df %>% mutate(line_variable_type2 = paste0(panel_variable_type_dec_nochange_inc_quotes, "(",round(panel_variable_value, digits = 4), ")"))
}

greening_incre_at_savanna <- si_grid_creation_prediction2(analyses_df_noNA_nooutliers, 
                                 "trend_annualtemp",
                                 "mean_savanna_percentage",
                                 "trend_re",
                                 "inc",
...
greening_decre_at_savanna <- si_grid_creation_prediction2(analyses_df_noNA_nooutliers, 
                                 "trend_annualtemp",
                                 "mean_savanna_percentage",
                                 "trend_re",
                                 "dec",
...
greening_nochangere_at_savanna <- si_grid_creation_prediction2(analyses_df_noNA_nooutliers, 
                                 "trend_annualtemp",
                                 "mean_savanna_percentage",
                                 "trend_re",
                                 "nochange",
                                 inc_gam)
greening_re_at_savanna <- bind_rows(greening_decre_at_savanna, greening_incre_at_savanna, greening_nochangere_at_savanna)
greening_re_at_savanna <- greening_re_at_savanna %>% mutate(Vegetation = "Savanna")

greening_incre_at_forest <- si_grid_creation_prediction2(analyses_df_noNA_nooutliers, 
                                 "trend_annualtemp",
                                 "mean_forest_percentage",
                                 "trend_re",
                                 "inc",
...
greening_decre_at_forest<- si_grid_creation_prediction2(analyses_df_noNA_nooutliers, 
                                 "trend_annualtemp",
                                 "mean_forest_percentage",
                                 "trend_re",
                                 "dec",
...
greening_nochangere_at_forest <- si_grid_creation_prediction2(analyses_df_noNA_nooutliers, 
                                 "trend_annualtemp",
                                 "mean_forest_percentage",
                                 "trend_re",
                                 "nochange",
                                 inc_gam)
greening_re_at_forest <- bind_rows(greening_decre_at_forest, greening_incre_at_forest, greening_nochangere_at_forest)
greening_re_at_forest <- greening_re_at_forest %>% mutate(Vegetation = "Forest")

greening_incre_at_grass <- si_grid_creation_prediction2(analyses_df_noNA_nooutliers, 
                                 "trend_annualtemp",
                                 "mean_grass_percentage",
                                 "trend_re",
                                 "inc",
...
greening_decre_at_grass <- si_grid_creation_prediction2(analyses_df_noNA_nooutliers, 
                                 "trend_annualtemp",
                                 "mean_grass_percentage",
                                 "trend_re",
                                 "dec",
...
greening_nochangere_at_grass <- si_grid_creation_prediction2(analyses_df_noNA_nooutliers, 
                                 "trend_annualtemp",
                                 "mean_grass_percentage",
                                 "trend_re",
                                 "nochange",
                                 inc_gam)
greening_re_at_grass <- bind_rows(greening_decre_at_grass, greening_incre_at_grass, greening_nochangere_at_grass)
greening_re_at_grass <- greening_re_at_grass %>% mutate(Vegetation = "Grass")

remove(greening_decre_at_grass, greening_incre_at_grass, greening_nochangere_at_grass,
                                  greening_decre_at_forest, greening_incre_at_forest, greening_nochangere_at_forest,
                                  greening_decre_at_savanna, greening_incre_at_savanna, greening_nochangere_at_savanna)
greening_re_at <- bind_rows(greening_re_at_forest, greening_re_at_grass, greening_re_at_savanna)

si_plot_trend_interaction2 <- function (interaction_df, xaxis_variable, xlabel){
  x_plot<- ggplot(interaction_df, aes(x = .data[[xaxis_variable]], y = main_effect,
                                          color = line_variable_type2, fill = line_variable_type2,
                                          group = (line_variable_type2))) +
    geom_line(aes(color = line_variable_type2), linewidth = 1) + 
    geom_ribbon(aes(ymin = lower.ci, ymax = upper.ci), alpha = 0.1) +
    coord_cartesian(xlim = c(quantile(interaction_df[[xaxis_variable]], probs=0.1),quantile(interaction_df[[xaxis_variable]], probs=0.9)), ylim = c(0,1)) +
    geom_hline(yintercept= greening_intercept_transposed, linetype='longdash', col = 'grey')+
    facet_wrap(factor(Vegetation, unique(interaction_df$Vegetation))
      ~factor(formation_level, unique(interaction_df$formation_level)), nrow=3 , ncol = 3)+
    theme_classic() +
    scale_fill_manual(values = c("#999999","#FF99FF" ,"#CC9966" ))+
    scale_color_manual(values=c("#000000","#660066",  "#663300" )) + 
    ylab ("Probability of greening") + xlab(xlabel)+
    theme(legend.title=element_blank())
  x_plot
}

plot_trend_veg_at_re <- si_plot_trend_interaction2 (greening_re_at, "trend_annualtemp", "trend in annual temperature (degree centigrade/year)")
ggsave (here("Outputs", "AnalysesResults", "GAMResults", "ThreeWay_InteractionResults", "SI_at_re_allveg.png"), 
        plot_trend_veg_at_re,
        dpi = 700, height = 20, width=20, units = "cm")
...
#for supplementary 
greening_trend_at <- greening_trend_at %>% mutate(TrajType = "Greening")
greening_trend_heatwave <- greening_trend_heatwave %>% mutate(TrajType = "Greening")
greening_trend_re <- greening_trend_re %>% mutate(TrajType = "Greening")
greening_trend_spei <- greening_trend_spei  %>% mutate(TrajType = "Greening")
...
uni_trend_at <- univariate_plotting("trend_annualtemp", "trend in annual\n temperature (degree centigrade/year)", greening_trend_at)
uni_trend_re <- univariate_plotting("trend_re", "trend in relative entropy \n (per year)", greening_trend_re)
uni_trend_heatwaves <- univariate_plotting("trend_heatwave", "trend in number of heatwaves (per year)", greening_trend_heatwave)
uni_trend_spei <- univariate_plotting("trend_spei", "trend in SPEI (per year)", greening_trend_spei)

si_climate_uni_plot <- ggpubr::ggarrange(plotlist = list(uni_trend_at, uni_trend_re, uni_trend_heatwaves, uni_trend_spei),
                               labels=c("a)", "b)", "c)", "d)"), ncol = 2, nrow = 2)
ggsave(here("Outputs", "AnalysesResults", "GAMResults", "SI_climate_uni_plot.png"),
       si_climate_uni_plot,dpi = 300, height = 20, width=20, units = "cm")
