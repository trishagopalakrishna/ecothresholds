...
#write_rds(analyses_df_noNA_nooutliers, here("Scripts", "Final_Scripts", "analyses", "analyses_df_noNA_nooutliers.rdata"))
...
univariate_relationships_list <- list(mean_at_plot_df, mean_re_plot_df, mean_burnedarea_plot_df, mean_spei_plot_df, 
                                     ...
                                     trend_at_plot_df, trend_re_plot_df, trend_ba_plot_df, trend_spei_plot_df, trend_heatwave_plot_df,
                                     anthropic_dist_plot_df)
...
#### TO DO Step 5c - Plotting maps of probability of greening/browning for a predictor variable while other predictors are at their mean value
```{r}
#plot of mean annual temperature effect
mapping_probability_function <- function (driver_variable, gam_model){
  ...
  df_estimates <- gratia::smooth_estimates(gam_model, data = df_to_predict, unconditional= T)
  ...
  only_probability <- df_estimates %>% dplyr::select(c(x,y, estimate_inv))
  x_rast <- rast(as.data.frame(only_probability %>% drop_na()), type="xyz")
  
  x_rast
}

Sys.time(); greening_mean_at_prob <- mapping_probability_function("mean_at", inc_gam); Sys.time()
Sys.time(); greening_trend_at_prob <- mapping_probability_function("trend_annualtemp", inc_gam); Sys.time()
Sys.time(); greening_mean_re_prob <- mapping_probability_function("mean_re", inc_gam); Sys.time()
Sys.time(); greening_trend_re_prob <- mapping_probability_function("trend_re", inc_gam); Sys.time()
Sys.time(); greening_mean_heatwave_prob <- mapping_probability_function("mean_heatwave", inc_gam); Sys.time()
Sys.time(); greening_trend_heatwave_prob <- mapping_probability_function("trend_heatwave", inc_gam); Sys.time()
Sys.time(); greening_mean_spei_prob <- mapping_probability_function("mean_spei", inc_gam); Sys.time()
Sys.time(); greening_trend_spei_prob <- mapping_probability_function("trend_spei", inc_gam); Sys.time()
Sys.time(); greening_mean_burnedarea_prob <- mapping_probability_function("mean_burnedarea", inc_gam); Sys.time()
Sys.time(); greening_trend_burnedarea_prob <- mapping_probability_function("trend_burnedarea", inc_gam); Sys.time()
Sys.time(); greening_mean_savanna_prob <- mapping_probability_function("mean_savannapercentage", inc_gam); Sys.time()
Sys.time(); greening_mean_grass_prob <- mapping_probability_function("mean_grasspercentage", inc_gam); Sys.time()
Sys.time(); greening_mean_forest_prob <- mapping_probability_function("mean_forestpercentage", inc_gam); Sys.time()
Sys.time(); greening_anthropicdist_prob <- mapping_probability_function("anthropic_dist", inc_gam); Sys.time()

Sys.time(); browning_mean_at_prob <- mapping_probability_function("mean_at", dec_gam); Sys.time()
Sys.time(); browning_trend_at_prob <- mapping_probability_function("trend_anualtemp", dec_gam); Sys.time()
Sys.time(); browning_mean_re_prob <- mapping_probability_function("mean_re", dec_gam); Sys.time()
Sys.time(); browning_trend_re_prob <- mapping_probability_function("trend_re", dec_gam); Sys.time()
Sys.time(); browning_mean_heatwave_prob <- mapping_probability_function("mean_heatwave", dec_gam); Sys.time()
Sys.time(); browning_trend_heatwave_prob <- mapping_probability_function("trend_heatwave", dec_gam); Sys.time()
Sys.time(); browning_mean_spei_prob <- mapping_probability_function("mean_spei", dec_gam); Sys.time()
Sys.time(); browning_trend_spei_prob <- mapping_probability_function("trend_spei", dec_gam); Sys.time()
Sys.time(); browning_mean_burnedarea_prob <- mapping_probability_function("mean_burnedarea", dec_gam); Sys.time()
Sys.time(); browning_trend_burnedarea_prob <- mapping_probability_function("trend_burnedarea", dec_gam); Sys.time()
Sys.time(); browning_mean_savanna_prob <- mapping_probability_function("mean_savannapercentage", dec_gam); Sys.time()
Sys.time(); browning_mean_grass_prob <- mapping_probability_function("mean_grasspercentage", dec_gam); Sys.time()
Sys.time(); browning_mean_forest_prob <- mapping_probability_function("mean_forestpercentage", dec_gam); Sys.time()
Sys.time(); browning_anthropicdist_prob <- mapping_probability_function("anthropic_dist", dec_gam); Sys.time()
