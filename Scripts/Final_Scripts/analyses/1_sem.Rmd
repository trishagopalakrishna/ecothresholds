...
all_data <- c(decrease,increase, anthropic_dist, burnedarea, hand, re, at, heatwave, spei, savannapercentage)
names(all_data)<- c("increase", "decrease", "anthropic_dist", "burnedarea", "hand", "re", "at", "heatwave", "spei", "savannapercentage")

df1 <- terra::as.data.frame(all_data, na.rm= T)

remove(anthropic_dist, burnedarea, hand, re, at, heatwave, spei, savannapercentage)
remove(ndvi_count_5km, decrease, increase)
```
...
```{r}
summary(df1)
inc_df_noNA <- df1 %>% dplyr::select(-c(decrease))
dec_df_noNA <- df1 %>% dplyr::select(-c(increase))
