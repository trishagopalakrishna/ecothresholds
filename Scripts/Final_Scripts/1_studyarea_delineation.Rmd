...
#To align with stability rasters at 0.05 degrees, resampling
ndvi_agg_stability<- rast(here("Outputs", "Final_TrajShape_Models", "Annual_NDVI", "agg_stability_masked_ndvi_11_trajresults.tif"))
mapbiomas_mask<- terra::resample(mapbiomas_mask, ndvi_agg_stability)
...
for (i in x_thresholds){
  ...
  writeRaster(threshold_mapbiomas_mask, here("Data", "Masks_to_define_pixelsofinterest", 
                                             "Mapbiomas_anthropic_mask","5km_thresholded_mapbiomas_masks", paste0("5km_thresholded_masks_",i, ".tif")))
...
#writeRaster(threshold_mapbiomas_mask_10, here("Data", "Masks_to_define_pixelsofinterest", "mapbiomas_anthropic_mask", "5km_thresholded_mapbiomas_mask_10.tif"))
Sys.time(); threshold_mapbiomas_mask_20<- terra::app(tp_mapbiomas_mask, fun= threshold_function); Sys.time()#1min
#writeRaster(threshold_mapbiomas_mask_20, here("Data", "Masks_to_define_pixelsofinterest", "mapbiomas_anthropic_mask", "5km_thresholded_mapbiomas_mask_20.tif"))
Sys.time(); threshold_mapbiomas_mask_30<- terra::app(tp_mapbiomas_mask, fun= threshold_function); Sys.time()#1min
#writeRaster(threshold_mapbiomas_mask_30, here("Data", "Masks_to_define_pixelsofinterest", "mapbiomas_anthropic_mask", "5km_thresholded_mapbiomas_mask_30.tif"))
remove(threshold_function)

threshold_map_function<- function (thresholded_raster, title){
  tm_shape(neighbors, bbox = map_extent)+ tm_borders()+
  tm_shape( threshold_raster)+
  tm_raster(style = 'cont',palette= get_brewer_pal("Reds"), n=5, breaks = seq(0,0.3,0.05), title=title) + 
              tm_layout (legend.position = c("left", "bottom"))+
              tm_facets(nrow=1, ncol=1, free.coords = FALSE)+
  tm_shape(d_trans)+ tm_borders()
}
...
map_thresh20_mapbiomas<- threshold_map_function(threshold_mapbiomas_mask_10, "20% threshold")
map_thresh30_mapbiomas<- threshold_map_function(threshold_mapbiomas_mask_10, "30% threshold")
