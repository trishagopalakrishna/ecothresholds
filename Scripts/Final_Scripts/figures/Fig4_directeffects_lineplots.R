...
#2. (i) For composite climate variables ti(trend_at, trend_re) & ti(trend_heatwaves, trend_spei)
composite_grid_creation_prediction <- function (df_analyses, 
...
greening_trend_at_re <- composite_grid_creation_prediction(analyses_df_noNA_nooutliers, 
                                                       "trend_annualtemp", 
                                                       "trend_re",
                                                       greening_gam,
                                                       "decreasing", "no change", "increasing")
browning_trend_at_re <- composite_grid_creation_prediction(analyses_df_noNA_nooutliers, 
                                                       "trend_annualtemp", 
                                                       "trend_re",
                                                       browning_gam,
                                                       "decreasing", "no change", "increasing")

greening_trend_heatwave_spei <- composite_grid_creation_prediction(analyses_df_noNA_nooutliers, 
                                                               "trend_heatwave", 
                                                               "trend_spei",
                                                               greening_gam,
                                                               "increasing", "no change", "decreasing")
browning_trend_heatwave_spei <- composite_grid_creation_prediction(analyses_df_noNA_nooutliers, 
                                                               "trend_heatwave", 
                                                               "trend_spei",
                                                               browning_gam,
                                                               "increasing", "no change", "decreasing")
...
remove(composite_combined_plot_df_prep, composite_grid_creation_prediction)
...
composite_plot_function <- function (interaction_trend_df, xaxis_variable, xaxis_label){
  x_plot<- ggplot(interaction_trend_df, aes(x = .data[[xaxis_variable]], y = main_effect,
    ...
    facet_wrap(~factor(line_variable_type, c("decreasing", "no change", "increasing")), nrow=1)+
...
composite_plot_trend_at_re <- composite_plot_function(composite_plot_df_list[[1]], "trend_annualtemp", "trend in annual temperature (degree centigrade/year)")
ggsave(here("Outputs", "TrendsResults", "Figures", "Fig3", "panel_re_at.png"),
       composite_plot_trend_at_re,
       dpi=700, width =15, height = 5, units = "cm")
composite_plot_trend_heatwave_spei <- composite_plot_function(composite_plot_df_list[[2]], "trend_heatwave", "trend in number of heatwaves (per year)")
ggsave(here("Outputs", "TrendsResults", "Figures", "Fig3", "panel_heatwave_spei.png"),
...
ggsave(here("Outputs", "TrendsResults", "Figures", "Fig3", "at_distribution.png"),
       at_density,
...
ggsave(here("Outputs", "TrendsResults", "Figures", "Fig3", "heatwave_distribution.png"),
       heatwave_density,
       dpi=700, width =15, height = 3, units = "cm")

panel_density_plot <- function (panel_term, panel_term_label){
  data_df2 <- analyses_df_noNA_nooutliers %>%
  dplyr::select(all_of(panel_term)) %>%
  dplyr::rename(z = 1)
density2 <- ggplot(data_df2, aes(x = z)) +
  geom_density(colour = "grey70", fill = "grey90", alpha = 0.6) +
  geom_vline(xintercept = mean(data_df2$z) - sd(data_df2$z), linetype="dashed") +
  geom_vline(xintercept = 0, linetype="dashed") +
  geom_vline(xintercept = mean(data_df2$z) + sd(data_df2$z), linetype="dashed") +
  theme_classic(base_size = 10) +
  ylab("kernel density") + xlab(panel_term_label)+ coord_flip()
}
re_density <- panel_density_plot("trend_re", "trend in relative entropy \n (per year)")
ggsave(here("Outputs", "TrendsResults", "Figures", "Fig3", "re_distribution.png"),
       re_density,
       dpi=700, width =5, height = 5, units = "cm")
spei_density <- panel_density_plot ("trend_spei", "trend in spei \n (per year)")
ggsave(here("Outputs", "TrendsResults", "Figures", "Fig3", "spei_distribution.png"),
       spei_density,
       dpi=700, width = 5, height = 5, units = "cm")
