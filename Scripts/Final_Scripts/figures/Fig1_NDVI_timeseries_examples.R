...
tmap_save(fig1_map, paste0(output_file_path, "/", "map1km_ndvi_aggtrajectories_monthly_swin11_2.png"),
          height = 16, width = 16, units = "cm", dpi= 400)
...
ggsave(here("Outputs", "TrajectoryPlotting", "maintext_NDVI_lineplots", "lin_inc.png"),
       fig_lin_inc, dpi = 300, height = 4, width = 4, units = "cm")
...
ggsave(here("Outputs", "TrajectoryPlotting", "maintext_NDVI_lineplots", "no_trend.png"),
       fig_no_trend, dpi = 300, height = 4, width = 4, units = "cm")
...
y_boxplot <- ggplot(byveg, aes(y = value, x = TrajType)) +
  ...
  xlab("Stability Dynamics") + ylab("Proportion of 1km pixel\ncovered by each vegetation formation")
ggsave(here("Outputs","TrendsResults", "Figures", "vegcomposition2.png"), y_boxplot,
...
monthly_ndvi_seperate_plot <- trajectorycomposition %>%
  ...
  theme(axis.text.x = element_text(angle = 45, vjust=0.7),
        ...
        axis.ticks.x = element_blank()) + ylab ("Proportional area (*1000 sqkm)")
ggsave(here("Outputs","TrendsResults", "Figures", "disaggregate_traj2.png"), monthly_ndvi_seperate_plot,
       dpi = 700, height = 15, width = 15, units = "cm")
