...
Sys.time(); monthly_map_ndvi_points<- points_map_withindex_function(monthly_rds_list[[3]], "NDVI"); Sys.time()
Sys.time(); monthly_map_evi_points<- points_map_withindex_function(monthly_rds_list[[2]], "EVI"); Sys.time()
Sys.time(); monthly_map_aniso_points<- points_map_withindex_function(monthly_rds_list[[1]], "anisoEVI"); Sys.time()

points_trajectory_map<- tmap_arrange(monthly_map_ndvi_points, monthly_map_evi_points, monthly_map_aniso_points, 
                                     ncol = 3, nrow = 1)
...
Sys.time(); annual_map_ndvi_points<- points_map_withindex_function(annual_rds_list[[3]], "NDVI"); Sys.time()
Sys.time(); annual_map_evi_points<- points_map_withindex_function(annual_rds_list[[2]], "EVI"); Sys.time()
Sys.time(); annual_map_aniso_points <- points_map_withindex_function(annual_rds_list[[1]], "anisoEVI"); Sys.time()

points_trajectory_map2<- tmap_arrange(annual_map_ndvi_points, annual_map_evi_points, 
                                      annual_map_aniso_points, ncol = 3, nrow = 1)
...
monthly_NDVItrajectories_split<- trajectory_segregate(monthly_rds_list[[3]])
monthly_EVItrajectories_split<- trajectory_segregate(monthly_rds_list[[2]])
monthly_anisoEVItrajectories_split <- trajectory_segregate(monthly_rds_list[[1]])

annual_NDVItrajectories_split<- trajectory_segregate(annual_rds_list[[3]])
annual_EVItrajectories_split<- trajectory_segregate(annual_rds_list[[2]])
annual_anisoEVItrajectories_split <- trajectory_segregate(annual_rds_list[[1]])
...
Sys.time(); monthly_aniso_distances <- calculate_distances(monthly_anisoEVItrajectories_split); Sys.time()
...
Sys.time(); annual_aniso_distances <- calculate_distances(annual_anisoEVItrajectories_split); Sys.time()
...
Sys.time(); monthly_aniso_closest_trajectory_df <- shortestdistance_trajectory(monthly_aniso_distances); Sys.time()
...
Sys.time(); annual_aniso_closest_trajectory_df <- shortestdistance_trajectory(annual_aniso_distances); Sys.time()
...
monthly_anisoEVItrajectories_split
annual_NDVItrajectories_split
annual_EVItrajectories_split
annual_anisoEVItrajectories_split
...
converted_monthly_anisotrajectories_split <- convert_NA_to_999_function(monthly_anisoEVItrajectories_split)
...
converted_annual_anisotrajectories_split <- convert_NA_to_999_function(annual_anisoEVItrajectories_split)
...
Sys.time(); converted_monthly_aniso_distances <- calculate_distances(converted_monthly_anisotrajectories_split); Sys.time()
...
Sys.time(); converted_annual_aniso_distances <- calculate_distances (converted_annual_anisotrajectories_split)
...
Sys.time(); converted_monthly_aniso_closest_trajectory_df <- shortestdistance_trajectory(converted_monthly_aniso_distances); Sys.time()
...
Sys.time(); converted_annual_aniso_closest_trajectory_df <- shortestdistance_trajectory(converted_annual_aniso_distances); Sys.time()
...
monthly_closest_aniso <- df_prep (monthly_aniso_closest_trajectory_df, "monthly_aniso")
...
annual_closest_aniso <- df_prep(annual_aniso_closest_trajectory_df, "annual_aniso")

closest_df<- left_join(monthly_closest_ndvi, monthly_closest_evi)
closest_df <- left_join(closest_df, monthly_closest_aniso)
closest_df <- left_join(closest_df, annual_closest_ndvi)
closest_df <- left_join(closest_df, annual_closest_evi)
closest_df <- left_join(closest_df, annual_closest_aniso)
closest_df$uniqueID<-as.numeric(closest_df$uniqueID)
pivot_closest_df <- pivot_longer (closest_df, 2:7)
...
within_studyarea_plot <- ggplot(pivot_closest_df, aes(x = annual_monthly, fill = value)) + 
  ...
  theme(legend.title=element_blank(),
        axis.text.x = element_text(angle= 45, vjust = 0.5, hjust=0.5)) +xlab("Index") +ylab ("Count") +
...
converted_monthly_aniso_closest_trajectory_df
names(converted_monthly_aniso_closest_trajectory_df) <- c("uniqueID", "monthly_aniso_mindist", "monthly_aniso")
...
converted_annual_aniso_closest_trajectory_df
names(converted_annual_aniso_closest_trajectory_df)<- c("uniqueID", "annual_aniso_mindist", "annual_aniso")

closest_df2 <- left_join(converted_monthly_NDVI_closest_trajectory_df, converted_monthly_EVI_closest_trajectory_df)
closest_df2 <- left_join(closest_df2, converted_monthly_aniso_closest_trajectory_df)
...
closest_df2 <- left_join(closest_df2, converted_annual_aniso_closest_trajectory_df)
...
trajectories_only<- closest_df3 %>% dplyr::select(c(uniqueID, monthly_NDVI, monthly_EVI, monthly_aniso, annual_NDVI, annual_EVI, annual_aniso))

pivot_closest_df2 <- pivot_longer (trajectories_only, 2:7)
...
#For SI
allsites_allindices_annualmonthly_barplot <- ggarrange(within_studyarea_plot, outside_studyarea_plot, nrow = 2, ncol = 1)
ggsave(here("Outputs", "AnalysesResults", "Relationship_to_fieldplots", "SI_allsites_allindices_annualmonthly_barplot.jpeg"),
       allsites_allindices_annualmonthly_barplot, dpi= 400, height = 30, width = 30, units = "cm")

mindist_df <- closest_df3 %>% dplyr::select(c(uniqueID, monthly_NDVI_mindist, 
                                              monthly_NDVI, monthly_EVI, monthly_aniso,
                                              annual_EVI, annual_NDVI, annual_aniso))
pivot_closest_df3 <- pivot_longer (mindist_df, 3:8)
...
p <- ggplot(pivot_closest_df3, aes(x = annual_monthly, y= monthly_NDVI_mindist, fill = value)) + 
  ...
  scale_fill_manual(values = c("#117733", "lightgrey","#CC6677", "#DDCC77" ,  "#882255",  "#88CCEE")) +
  theme_classic(base_size = 14) + 
  xlab ("Trajectory type") + ylab ("Closest distance to (m)") +
  theme(legend.position = "none",
        axis.text.x = element_text(angle=45, vjust=0.5, hjust=0.5)) + 
  ggtitle ("Distribution of distance of field study to closest trajectory")
ggsave(here("Outputs", "AnalysesResults", "Relationship_to_fieldplots", "mindist_outfieldsites_allindices_annualmonthly.jpeg"),
