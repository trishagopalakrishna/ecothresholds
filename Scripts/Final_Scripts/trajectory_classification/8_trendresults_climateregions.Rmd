...
(2) Data analyses of the distribution of seperate trajectories and aggregated trajectories across all indices in the study area that is present in each of the climate zones 
...
sep_df_prep_function <- function (sep_traj_raster){
  ...
  for (i in 1:length(climate_trans$region)){
        x_region_raster <- terra::crop(reclass_trajectory_results_raster , vect(climate_trans$geometry[[i]]))
        x_region_raster <- terra::mask(x_region_raster, vect(climate_trans$geometry[[i]]))
        ...
        x_area <- x_area %>% mutate( Region = climate_trans$region[i])
  ...
  for (i in 1:length(climate_trans$region)){
      ...
      for (j in 1:length(trajectory_names)){
        x_region_raster <- terra::crop(split_trajectory[[j]], vect(climate_trans$geometry[[i]]))
        x_region_raster <- terra::mask(x_region_raster, vect(climate_trans$geometry[[i]]))
        area_raster <- terra::cellSize(x_region_raster, unit = "km", mask= T)
        x_area <- sum(values(area_raster), na.rm = T)
        x_area <- as.data.frame(x_area)
        names(x_area) <- "area_sqkm"
        x_area <- x_area %>% mutate(TrajectoryType = trajectory_names[j], Region = climate_trans$region[i])
        list_area[[j]]<- x_area
     }
...
x<- ggplot(monthly_agg_ndvi_area, aes(x=factor(Region), y= area_sqkm/1000, fill = factor(TrajectoryType)))+geom_bar(stat = "identity")
...
plotting_function <- function (df){
  ...
  ggplot(df, aes(x = factor(Region), y= area_sqkm/1000, fill = factor(TrajectoryType))) + 
   geom_bar(stat = "identity") + theme_classic() +
    scale_fill_manual(values =aggregate_trajectory_palette)+
    theme(legend.position="none")+
  theme(axis.title.x = element_blank(),
    axis.ticks.x = element_blank()) + ylab ("Proportion of study area") 
}  
...
agg_barplot <- ggplot(df_agg, aes(y = area_sqkm/1000, x = factor(Region), fill =factor(TrajectoryType))) +
