...
```
#Introduction
In this script, I evaluate the climate regions that the different trajectories fall into. 
The climate regions are from Catellan et al 2024 
(1) Read final trajectory type results for all indices, aggregated trajectories for all indices and climate regions GIS files
(2) Data analyses of the distribution of seperate trajectories and aggregated trajectories across all indices in each of the climate zones 
...
#climate regions
climate_trans<- st_read(here ("Data", "Cattelanetal_Clustering", "cerrado_climate_zones.shp"))
climate_trans<- st_transform(climate_trans, crs = 4326)
climate_trans<- climate_trans %>% mutate(area_km= terra::expanse(vect(climate_trans), unit="km"))
cerrado<- climate_trans %>% st_union()
...
area_separate_trajectory_calculation <- function (trajectory_results_raster){
  ...
  df_for_histogram<- list()
   for (i in 1:9){
     list_area <- list()
     for (j in 1:length(climate_trans$region)){
       x_region_raster <- terra::crop(split_trajectory[[i]], vect(climate_trans$geometry[[j]]))
       x_region_raster <- terra::mask(x_region_raster, vect(climate_trans$geometry[[j]]))
       ...
       x_area <- x_area %>% mutate(TrajectoryType = trajectory_names[i], Region = climate_trans$Region[j])
...
Sys.time(); monthly_seperate_aniso_area <- area_separate_trajectory_calculation(seperate_monthly_rds_list[[1]]); Sys.time()
Sys.time(); monthly_seperate_evi_area <- area_separate_trajectory_calculation(seperate_monthly_rds_list[[2]]); Sys.time()
Sys.time(); monthly_seperate_ndvi_area <- area_separate_trajectory_calculation(seperate_monthly_rds_list[[3]]); Sys.time()

Sys.time(); annual_seperate_aniso_area <- area_separate_trajectory_calculation(seperate_annual_rds_list[[1]]); Sys.time()
Sys.time(); annual_seperate_evi_area <- area_separate_trajectory_calculation(seperate_annual_rds_list[[2]]); Sys.time()
Sys.time(); annual_seperate_ndvi_area <- area_separate_trajectory_calculation(seperate_annual_rds_list[[3]]); Sys.time()
...
Sys.time(); monthly_agg_aniso_area <- area_agg_trajectory_calculation(agg_monthly_rds_list[[1]]); Sys.time()
Sys.time(); monthly_agg_evi_area <- area_agg_trajectory_calculation(agg_monthly_rds_list[[2]]); Sys.time()
Sys.time(); monthly_agg_ndvi_area <- area_agg_trajectory_calculation(agg_monthly_rds_list[[3]]); Sys.time()

Sys.time(); annual_agg_aniso_area <- area_agg_trajectory_calculation(agg_annual_rds_list[[1]]); Sys.time()
Sys.time(); annual_agg_evi_area <- area_agg_trajectory_calculation(agg_annual_rds_list[[2]]); Sys.time()
Sys.time(); annual_agg_ndvi_area <- area_agg_trajectory_calculation(agg_annual_rds_list[[3]]); Sys.time()

```


#Step 4- visualization of results - bar plots
```{r}
climate_color_palette <- c("#6699ff", "#33ccff", "#000066")

plotting_function <- function (df){
  
  ggplot(df, aes(x = Region, y= Percentage, fill =Region)) + 
   geom_bar(stat = "identity") + theme_classic() +
    scale_fill_manual(values =climate_color_palette)+
...
write_rds(plot_monthly_ndvi_agg , here("Outputs", "TrendsResults", "Figures", "agg_trajectories_climatezones.rdata"))
ggsave(here("Outputs", "TrendsResults", "barplot_agg_ndvi_monthly_climateregions.png"),
...
ggsave(here("Outputs", "TrendsResults", "barplot_agg_climateregions_indices_timestep.png"),
