...
(4) Visualization of results using boxplots only for aggregated trajectories
...
resample_mean_vegformation <- function (veg_formation){
  x_resample <- terra::resample(veg_formation, seperate_monthly_rds_list[[3]], method = "bilinear")
  x_resample
}
...
studyarea_veg_formation <- terra::mask (resample_veg_formation, seperate_monthly_rds_list[[3]]) #crop and mask to trajectory result rasters
```
...
Sys.time(); monthly_aniso_seperate <- seperate_trajectory_veg(seperate_monthly_rds_list[[1]]); Sys.time()
Sys.time(); monthly_evi_seperate <- seperate_trajectory_veg(seperate_monthly_rds_list[[2]]); Sys.time()
Sys.time(); monthly_ndvi_seperate <- seperate_trajectory_veg(seperate_monthly_rds_list[[3]]); Sys.time()

Sys.time(); annual_aniso_seperate <- seperate_trajectory_veg(seperate_annual_rds_list[[1]]); Sys.time()
Sys.time(); annual_evi_seperate <- seperate_trajectory_veg(seperate_annual_rds_list[[2]]); Sys.time()
Sys.time(); annual_ndvi_seperate <- seperate_trajectory_veg(seperate_annual_rds_list[[3]]); Sys.time()
...
Sys.time(); monthly_aniso_agg <- aggregate_trajectory_veg(agg_monthly_rds_list[[1]]); Sys.time()
Sys.time(); monthly_evi_agg <- aggregate_trajectory_veg(agg_monthly_rds_list[[2]]); Sys.time()
Sys.time(); monthly_ndvi_agg <- aggregate_trajectory_veg(agg_monthly_rds_list[[3]]); Sys.time()

Sys.time(); annual_aniso_agg <- aggregate_trajectory_veg(agg_annual_rds_list[[1]]); Sys.time()
Sys.time(); annual_evi_agg <- aggregate_trajectory_veg(agg_annual_rds_list[[2]]); Sys.time()
Sys.time(); annual_ndvi_agg <- aggregate_trajectory_veg(agg_annual_rds_list[[3]]); Sys.time()

```

#Step 4- visualization of results - boxplots for aggregated trajectories only
```{r}
...
write_rds(df_monthly_ndvi_agg, here("Outputs", "TrendsResults", "Figures", "agg_trajectories_vegformations_boxplot.rdata"))
...
vegformation_palette<- c("#117733", "#CC6699", "#CC9933" )
