...
trendresults_barplot<- plot_df %>% 
  ...
  xlab("Model Type")
ggsave(here(output_file_path, paste0("barplot_swin11_seperate_trajectories.png")),
...
plot_df <- plot_df %>% mutate(value = case_when(value== 1~ "Decrease",
                                                value==2~ "Increase",
                                                  TRUE~"No trend"))
plot_df <- plot_df  %>%  separate_wider_delim(index, delim = "_", names =c("Index", "TimeStep"))
...
#Step 6a (for results section) - calculating proportions of seperate indices within the increase and decrease aggregated trajectories
```{r}
...
proportion_df_calc <- function (trendresults_raster, index_name){
  ...
  x_df<- x_df %>% mutate(index = index_name)
  names(x_df)[4]<- "value"
  x_df <- x_df %>% mutate(seperate_traj = case_when(value== 1~ "Linear decrease",
                                                value==2~ "Linear increase",
                                                value==4~ "Step decrease",
                                                value==5~"Step increase",
                                                value==6~"Quadratic decrease (accelerated)",
                                                ...
                                                value==9~"Quadratic increase (decelerated)",
                                                TRUE~"No trend"))
  x_df <- x_df %>% mutate(agg_traj = case_when(value== 1~ "Decrease",
                                                value==2~ "Increase",
                                                value==4~ "Decrease",
                                                value==5~"Increase",
                                                value==6~"Decrease",
  ...
  seperate_traj_count_df <- seperate_traj_count_df %>% mutate(percentage = (count/total_count)*100)
...
#Step 6b (for results section) - calculating average difference in results (aggregated trajectories) between monthly and annual timesteps across all indices
```{r}
...
dec_ndvi <- difference_function("Decrease","NDVI")
dec_evi <- difference_function("Decrease","EVI")
dec_aniso <- difference_function("Decrease","anisoEVI")
mean_dec_all_indices <- mean(c(dec_ndvi, dec_evi, dec_aniso))

inc_ndvi <- difference_function("Increase","NDVI")
inc_evi <- difference_function("Increase","EVI")
inc_aniso <- difference_function("Increase","anisoEVI")
