...
However the main problem is that the geographic area of interest (inside the Cerrado) 
is not the same when looking at the result maps of anisoEVI and NDVI! And this is the main reason 
for NDVI being huge rasters, having more nrows than anisoEVI etc

Firstly, the water bodies have not been excluded from the MODIS NDVI product! I looked at my
original script in GEE and yes I have NOT excluded water bodies, similar to anisoEVI. 
AnisoEVI inherently masked out water bodies and then created the anisoEVI values per remaining
pixels, which is not the case for NDVI. Next reason is that when I decided to keep only pixels 
that have less than 4 consecutive missing monthly values, a lot of pixels in anisoEVI got 
excluded (especially in the north), but a lot of pixels in the NDVI still remained because
it clearly has no consecutive missing values. 

So moving ahead I have retained only pixels from NDVI that are in the anisoEVI final results. 
This way the results for NDVI (area barplot,maps) and combination results with anisoEVI will 
be consistently including same geographic area of interest (ie no water bodies and only 
pixels with less than 4 missing consecutive monthly anisoEVI values)
...
```
##Results compilation-Result 8- from the final results after masking using anisoEVI for both indices, 
I exclude anthropic area processed in 1_studyarea_delineation.Rmd
```{r}
anisoEVI_map_results<- rast(here("Outputs", "Final_TrajShape_Models", "Annual", "am_trends11_trajresults.tif"))
#from lines 808-823. 1- Linear decrease, 2- linear increase, 3- no trend, 4- step decrease, 5- step increase
...
##Result 9- After excluding anthropic area from the results of all indices, I complete 
(1) cross tab analyses between NDVI & EVI, (2) make a map of difference pixels between NDVI & EVI and 
(3) find where and how much pixels match across all three indices
...
# Aggregation of trend resutls for all indices (excluding anthropic area masking)
to 5 x 5km resolution
To think about stability as the framing and not resilience, I have decided
to aggregate linear, step and quadratic (if any) to one class ie "unstable" pixels
and no trend to "stable" pixels. Then I aggregate by calculating the proportion of
the larger 5x5km pixel that is stable. 
...
##### Reclassification into stable and unstable pixels where stable is 1 and unstable is 2
m <- rbind(c(1, 2), c(2, 2), c(3,1), c(4,2), c(5,2), c(6,2), c(7,2))

classify_func<- function(raster){
  terra::classify(raster, m, others=NA)
}

anisoEVI_stability<- classify_func(anisoEVI_map_results)
#writeRaster(anisoEVI_stability,here("Outputs", "Final_TrajShape_Models", "Annual", "stability_am_11_trajresults.tif") )
ndvi_stability<- classify_func(masked_ndvi_results)
#writeRaster(ndvi_stability,here("Outputs", "Final_TrajShape_Models", "Annual_NDVI", "stability_masked_ndvi_11_trajresults.tif") )
evi_stability<- classify_func(masked_evi_results)
#writeRaster(evi_stability,here("Outputs", "Final_TrajShape_Models", "Annual_EVI", "stability_masked_evi_11_trajresults.tif") )

##### Aggregation of stable/unstable to 5 x 5km 
mat<- rast(here("Outputs", "OtherVariables", "Climate", "mat_1981_2021.tif")) #from GEE
prop_aggregate <- function(x) {
  terra::project(
    terra::segregate(x), mat,
    method = "average", res = res(mat)[1]
  )
}

anisoEVI_agg_stability<- prop_aggregate(anisoEVI_stability)
writeRaster(anisoEVI_agg_stability[[1]],here("Outputs", "Final_TrajShape_Models", "Annual", "agg_stability_am_11_trajresults.tif"), overwrite=T)
ndvi_agg_stability<- prop_aggregate(ndvi_stability)
writeRaster(ndvi_agg_stability[[1]],here("Outputs", "Final_TrajShape_Models", "Annual_NDVI", "agg_stability_masked_ndvi_11_trajresults.tif"), overwrite=T)
evi_agg_stability<- prop_aggregate(evi_stability)
writeRaster(evi_agg_stability[[1]], here("Outputs", "Final_TrajShape_Models", "Annual_EVI", "agg_stability_masked_evi_11_trajresults.tif"), overwrite=T)
...
map_function<- function(raster, title){
  tm_shape(cerrado)+ tm_fill(title="")+
  tm_shape (raster)+
  tm_raster(style = "cont", palette ="Greens", title=title)  
}

map_anisoEVI_stability<- map_function(anisoEVI_agg_stability[[1]], "AnisoEVI")
map_ndvi_stability<- map_function(ndvi_agg_stability[[1]], "NDVI")
map_evi_stability<- map_function(evi_agg_stability[[1]], "EVI")

aggregate_stability<- tmap_arrange(map_anisoEVI_stability, map_ndvi_stability, map_evi_stability)
tmap_save(aggregate_stability, here("Outputs", "Final_TrajShape_Models", "map_stability_agg_allindices.png"),
...
thresh_aniso_stability<- terra::mask(anisoEVI_agg_stability[[1]], threshold)
thresh_ndvi_stability<- terra::mask(ndvi_agg_stability[[1]], threshold)
thresh_evi_stability<- terra::mask(evi_agg_stability[[1]], threshold)

thresh_map_anisoEVI_stability<- map_function(thresh_aniso_stability, "AnisoEVI")
thresh_map_ndvi_stability<- map_function(thresh_ndvi_stability, "NDVI")
thresh_map_evi_stability<- map_function(thresh_evi_stability, "EVI")

thresh_aggregate_stability<- tmap_arrange(thresh_map_anisoEVI_stability, thresh_map_ndvi_stability, thresh_map_evi_stability, nrow=1, ncol=3)
tmap_save(thresh_aggregate_stability, here("Outputs", "Final_TrajShape_Models", "thresholded_map_stability_agg_allindices.png"),
...
# Calculation of area at each step of analyses
