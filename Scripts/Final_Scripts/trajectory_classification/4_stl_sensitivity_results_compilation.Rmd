...
plot_df_prep_function2 <- function (results_raster_list){
  ...
  all_percentage_results <- bind_rows(percentage_results_df_list)
  all_percentage_results <- all_percentage_results %>% mutate(value = case_when(value== 1~ "Browning",
                                                                                value== 2~ "Greening",
                                                                                value== 3~ "No trend"))
  all_percentage_results
}  
...
ggsave(here("Outputs","TrendsResults","annual_monthly_comparison","aggtrajectories_barplot_timstep_stlsetting_indicex.png"),
       x,dpi = 700, height = 60, width=60, units = "cm")
...
monthly_dec_ndvi <- difference_function("monthly","Browning","NDVI")
monthly_dec_evi <- difference_function("monthly","Browning","EVI")
monthly_dec_aniso <- difference_function("monthly","Browning","anisoEVI")
mean_monthly_dec_all_indices <- mean(c(monthly_dec_ndvi , monthly_dec_evi, monthly_dec_aniso))

annual_dec_ndvi <- difference_function("annual","Browning","NDVI")
annual_dec_evi <- difference_function("annual","Browning","EVI")
annual_dec_aniso <- difference_function("annual","Browning","anisoEVI")
mean_annual_dec_all_indices <- mean(c(annual_dec_ndvi , annual_dec_evi, annual_dec_aniso))

monthly_inc_ndvi <- difference_function("monthly","Greening","NDVI")
monthly_inc_evi <- difference_function("monthly","Greening","EVI")
monthly_inc_aniso <- difference_function("monthly","Greening","anisoEVI")
