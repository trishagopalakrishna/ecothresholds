...
mapping_function <- function (anomoly_raster, title){
  x_map<- tm_shape(cerrado)+ tm_borders()+
    ...
    tm_raster(n=5, style = "quantile",palette = "YlOrBr", title =title) 
    tm_layout(legend.text.size = 1, title.size = 4) +
  x_map
}
Sys.time();std_map <- mapping_function (std_burnedarea, "Std"); Sys.time()
Sys.time();lm_map <- mapping_function (lm_slope_burnedarea, "Linear Regression"); Sys.time()
Sys.time();tau_map <- mapping_function (kendall_tau_burnedarea , "Mann-Kendall"); Sys.time()

anomoly_results_map<- tmap_arrange(std_map, lm_map, tau_map, ncol =3, nrow= 1) 
tmap_save(anomoly_results_map, here("Outputs", "OtherVariables", "Fire", "anomoly_methods_results.png"),
