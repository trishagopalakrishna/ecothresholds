...
```
#Introduction
In this script, I evaluate the fire regions that the different trajectories fall into. 
The fie regions are from Segura Garcia et al., 2025. I specifically focus on period 4 from Segura- Garcia 2025 which spans
2012-2020, so sort of overlaps the time period of this study.
(1) Read final trajectory type results for all indices, aggregated trajectories for all indices and the fire regions GIS files
...
The results from this script were ultimately not included in the manuscript. 
...
#fire regions- period 4 from Segura Garcia 2025
fire_p4<- st_read(here ("Data", "Fire", "Segura_Garciaetal2024", "constClusteringClass_3solutions_grid30km_period4", "constClusteringClass_3solutions_grid30km_period4.shp"))
fire_p4_proj<- sf::st_transform(fire_p4, crs(agg_monthly_rds_list[[3]]))
...
fire_p4_proj_crop<- st_intersection(fire_p4_proj, cerrado)
...
sep_df_prep_function <- function (sep_traj_raster){
  ...
  for (i in 1:length(fire_p4_proj_crop$region_cd)){
        x_region_raster <- terra::crop(reclass_trajectory_results_raster , vect(fire_p4_proj_crop$geometry[[i]]))
        x_region_raster <- terra::mask(x_region_raster, vect(fire_p4_proj_crop$geometry[[i]]))
        ...
        x_area <- x_area %>% mutate( Region = fire_p4_proj_crop$region_cd[i])
  ...
  for (i in 1:length(fire_p4_proj_crop$region_cd)){
      ...
      for (j in 1:length(trajectory_names)){
        x_region_raster <- terra::crop(split_trajectory[[j]], vect(fire_p4_proj_crop$geometry[[i]]))
        x_region_raster <- terra::mask(x_region_raster, vect(fire_p4_proj_crop$geometry[[i]]))
        area_raster <- terra::cellSize(x_region_raster, unit = "km", mask= T)
        x_area <- sum(values(area_raster), na.rm = T)
        x_area <- as.data.frame(x_area)
        names(x_area) <- "area_sqkm"
        x_area <- x_area %>% mutate(TrajectoryType = trajectory_names[j], Region = fire_p4_proj_crop$region_cd[i])
        list_area[[j]]<- x_area
     }
...
```{r}
write_rds(monthly_agg_ndvi_area , here("Outputs", "TrendsResults", "Figures", "agg_trajectories_fireregions.rdata"))

aggregate_trajectory_palette <- c("#663300", "#61A36A", "#7C7083")

plotting_function <- function (df){
  ggplot(df, aes(x = reorder(factor(Region, 
...
ggsave(here("Outputs", "TrendsResults", "barplot_agg_ndvi_monthly_fireregions.png"),
...
agg_barplot <- ggplot(df_agg, aes(y = area_sqkm/1000, x = reorder(factor(Region, 
                                        levels = c("LS_P4", "HFA_P4", "LFA_P4")), -(area_sqkm/1000)), fill =factor(TrajectoryType))) +
 ...
 scale_fill_manual(values =aggregate_trajectory_palette)  + ylab ("Area (*1000 sqkm)") 
ggsave(here("Outputs", "TrendsResults", "barplot_agg_fireregions_indices_timestep.png"),
