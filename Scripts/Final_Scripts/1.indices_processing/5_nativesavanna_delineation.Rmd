...
#Step 3- study area figures (1 and 5km scale) for SI
```{r}
neighbors<- st_read(here("Data", "Admin", "brazil_neighbors.shp"))
map_extent<- st_bbox(c(xmin=-63.63, xmax=-24.73,
                       ymin= -0.81, ymax=-33.20), crs=4326) %>% st_as_sfc()
...
reclass_threshold20_1km <- terra::classify(threshold20_1km, rclmat, include.lowest= TRUE)
reclass_threshold20_5km <- terra::classify(threshold20_5km, rclmat, include.lowest= TRUE)

mapping_function <- function (trendresults_raster, title){
  #tm_shape(neighbors, bbox = map_extent)+ tm_borders()+
    tm_shape(cerrado)+ tm_borders()+
    tm_shape (trendresults_raster) + 
    tm_raster(col.scale = tm_scale_categorical(n=1, values = "#9f8f12"), col.legend = tm_legend_hide()) +
    tm_title_in(title, size = 5, position = tm_pos_in("left", "top"))
}

mask_1km <- mapping_function(reclass_threshold20_1km, "a)")
mask_5km <- mapping_function(reclass_threshold20_5km, "b)")

mask_maps <- tmap_arrange(mask_1km, mask_5km, ncol =2, nrow= 1)

tmap_save(mask_maps, here("Data", "Masks_to_define_pixelsofinterest", "mapbiomas_anthropic_mask", "SI_studyarea.png"),
